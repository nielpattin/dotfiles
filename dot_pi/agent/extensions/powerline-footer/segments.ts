...
const thinkingSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    // Keep fixed single color regardless of level
...
const sessionSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    const rawName = ctx.sessionName?.trim();
    if (rawName) {
      // For named sessions, show label directly (no explicit truncation)
      return { content: rawName, visible: true };
    }

    // Fallback to short session id when no name is set
