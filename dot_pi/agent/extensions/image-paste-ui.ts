...
const QUOTED_POSIX_IMAGE_PATH_RE = /(["'])(~?\/(?:\\.|[^"'\\])+?\.(?:png|jpe?g|gif|webp))\1/gi;
const QUOTED_WINDOWS_IMAGE_PATH_RE = /(["'])([a-zA-Z]:[\\/](?:\\.|[^"'\\])+?\.(?:png|jpe?g|gif|webp))\1/gi;
const BARE_POSIX_IMAGE_PATH_RE =
  /(?:^|[\s`(])((?:~\/|\/)(?:\\.|[^\s"'`(),])+?\.(?:png|jpe?g|gif|webp))(?=$|[\s"'`),.;:!?])/gi;
const BARE_WINDOWS_IMAGE_PATH_RE =
  /(?:^|[\s`(])([a-zA-Z]:[\\/](?:\\.|[^\s"'`(),])+?\.(?:png|jpe?g|gif|webp))(?=$|[\s"'`),.;:!?])/gi;
...
function extractImagePaths(text: string): Array<{ rawPath: string; resolvedPath: string; }> {
  ...
  QUOTED_POSIX_IMAGE_PATH_RE.lastIndex = 0;
  for (const match of text.matchAll(QUOTED_POSIX_IMAGE_PATH_RE)) {
    addCandidate(match[2]);
  }

  QUOTED_WINDOWS_IMAGE_PATH_RE.lastIndex = 0;
  for (const match of text.matchAll(QUOTED_WINDOWS_IMAGE_PATH_RE)) {
    addCandidate(match[2]);
  }

  BARE_POSIX_IMAGE_PATH_RE.lastIndex = 0;
  for (const match of text.matchAll(BARE_POSIX_IMAGE_PATH_RE)) {
    addCandidate(match[1]);
  }

  BARE_WINDOWS_IMAGE_PATH_RE.lastIndex = 0;
  for (const match of text.matchAll(BARE_WINDOWS_IMAGE_PATH_RE)) {
    addCandidate(match[1]);
  }
