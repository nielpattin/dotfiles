...
2. **Invoke Oracle** to review breakdown and suggest improvements
   - Review the **project plan quality** (scope, sequencing, dependencies, risks, validation)
   - Use local context first: plan file + repo context + `overseer` and `overseer-plan` skill references
   - Do **not** research Overseer hierarchy/API semantics on the web
   - External research is allowed only for missing **project-domain** facts needed for review
   - If external research is used, report: `External research used: yes/no` and `Reason`
...
**Note**: Priority must be 0-2 (0 highest). Blockers cannot be ancestors or descendants.
