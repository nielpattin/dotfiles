...
```
Review this PROJECT PLAN breakdown for "<milestone>":

Scope rules:
- Focus on project plan quality (scope, sequencing, dependencies, risks, validation).
- Use local-first context: current plan file, repository context, and local Overseer skill docs.
- Do NOT do web research for Overseer hierarchy/API semantics.
- Web research is allowed only if a required project-domain fact is missing locally.
- If external research is used, include: "External research used: yes/no" and "Reason".
...
Return concise actionable feedback. Do not ask the user questions.
```
