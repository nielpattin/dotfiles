...
### 6. Quarto & Presentation Specifics
- **Font Settings**: For diagrams used in slides, specify `defaultFontFamily="Noto Sans JP"` in `mxGraphModel` and explicitly in each text element's style attribute.
- **Background**: Remove `background="#ffffff"` to allow transparency.
- **Font Size**: Use 1.5x standard font size (around 18px) for PDF/Presentation readability.
- **Layering**: Always place arrows at the back (positioned early in the XML after the title) to avoid overlapping labels.
- **Margins**: Internal elements must have at least 30px margin from background frame boundaries.
...
This skill includes reference documents in the `reference/` folder. **Read these files when working on the corresponding diagram types:**

| Reference File | When to Read |
|----------------|--------------|
| `reference/drawio-shape-reference.md` | **Always read first** - Quick lookup for shapes, colors, connectors, and sizing |
| `reference/pmp-formulas-reference.md` | When creating EVM charts, PERT diagrams, or any PMP calculation visualizations |
| `reference/raci-matrix-template.md` | When creating RACI matrices - contains templates and examples |
| `reference/project-wbs-odoo-implementation.md` | When creating WBS diagrams - contains structure examples |
| `reference/project-charter-template.md` | When creating project charter visuals or diagrams |
| `reference/bir-tax-compliance-workflow.md` | Example of a compliance flowchart with swimlanes |
| `reference/insightpulse-architecture.md` | Example of a system architecture diagram |
| `reference/aws-icons.md` | When using AWS icons and service symbols |
| `reference/layout-guidelines.md` | Quick reference for margins, fonts, and layering |

### How to Use Reference Files

1. **Before creating any diagram**, read `reference/drawio-shape-reference.md` and `reference/layout-guidelines.md` for style lookups
2. **For AWS-related diagrams**, refer to `reference/aws-icons.md` for the latest `mxgraph.aws4.*` styles
3. **For PMP/PMBOK diagrams**, also read `reference/pmp-formulas-reference.md` for accurate formulas
3. **For specific diagram types**, read the corresponding template file for structure guidance
4. **Use examples** from `reference/bir-tax-compliance-workflow.md` and `reference/insightpulse-architecture.md` as patterns
