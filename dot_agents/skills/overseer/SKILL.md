...
// 5. Complete with learnings (VCS required - AI-generated conventional commit + bubbles learnings to parent)
await tasks.complete(task.id, {
  ...
  commitBodyBullets: [
    ...
    "cover login success/failure tests"
  ]
});
...
// 6. Optional: archive only when user asks or cleanup is intentional
// await tasks.archive(task.id);

// Keep completed tasks visible by default during active verification/debugging.
```
