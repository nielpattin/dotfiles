...
- Manually tested valid/invalid credentials`,
  ...
  commitHeader: "feat(auth): implement login endpoint",
  commitBodyBullets: [
    "add JWT issuance flow",
    "handle invalid credential path",
    "add coverage for login success/failure"
  ]
});
```

**VCS Required:** Uses provided `commitHeader`/`commitBodyBullets` when present; otherwise generates a conventional commit message. Commits changes (NothingToCommit treated as success), preserves task bookmark/branch, and fails with `NotARepository` if no git repo is found.
...
### Visibility + Commit Location Notes

- Completed tasks remain visible unless explicitly archived.
- If archived, default task list/UI views may hide them; include archived tasks to inspect history.
- Completion commit is created on the task branch (`task/<id>`), not on `main`.
- Overseer does not auto-merge/squash into `main`; merge strategy is manual.
...
| `tasks.complete(id)` | **VCS required** - commit uses optional `commitHeader`/`commitBodyBullets` (or auto-generated fallback), preserves task bookmark/branch |
| `tasks.complete(milestoneId)` | Same behavior for milestone completion; descendant branches are preserved |
