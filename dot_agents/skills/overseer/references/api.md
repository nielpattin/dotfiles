...
| `complete` | `Task` | **VCS required** - commits with optional `commitHeader`/`commitBodyBullets` + bubbles learnings to parent |
...
| `tasks.complete(id)` | **VCS required** - uses provided commit header/bullets when set, otherwise auto-generates conventional commit; branch preserved |
...
// Complete task with learnings (VCS required - AI-generated conventional commit, bubbles learnings to parent)
await tasks.complete(subtask.id, {
  result: "Implemented using jose library",
  learnings: ["Use jose instead of jsonwebtoken"],
  commitHeader: "feat(auth): implement token refresh",
  commitBodyBullets: [
    "add refresh token validation",
    "cover expiry and rotation tests"
  ]
});
