...
graph TD
    ...
    B --> C{Auth}
    C -->|Success| D[Session Manager]
    C -->|Failure| E[Disconnect]
    D --> F[Broadcast Engine]
\`\`\`
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/plan')
      ...
      .then((data: { plan: string; origin?: 'opencode' }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Include selected model for OpenCode
      if (modelSettings.providerID && modelSettings.modelID) {
        body.model = {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (showExport || showFeedbackPrompt || pendingPasteImage) return;

      // Don't intercept if already submitted or submitting
      if (submitted || isSubmitting) return;

      // Don't intercept in demo mode (no API)
      if (!isApiMode) return;
    ...
    showExport, showFeedbackPrompt, pendingPasteImage,
      ...
      <div className="h-screen flex flex-col bg-background overflow-hidden">
        ...
        <header className="h-12 flex items-center justify-between px-2 md:px-4 border-b border-border/50 bg-card/50 backdrop-blur-xl z-50">
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <ModeToggle />
            <Settings taterMode={taterMode} onTaterModeChange={handleTaterModeChange} origin={origin} />
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto bg-grid">
            <div className="min-h-full flex flex-col items-center px-4 py-3 md:px-10 md:py-8 xl:px-16">
              {/* Mode Switcher */}
              <div className="w-full max-w-208 2xl:max-w-5xl mb-3 md:mb-4 flex justify-start">
